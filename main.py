...
st.markdown("""
    ...
    .card-hover-image {
        display: none; position: fixed; left: 30px; top: 50px; z-index: 999999 !important;
    ...
    hr { margin: 0.3rem 0 !important; border: 0; border-top: 1px solid rgba(255,255,255,0.1) !important; }
    </style>
...
def load_data():
    try:
        ...
        df['is_unique'] = df['name'].str.contains('^\\*') | df.get('unique', False)
...
def render_card_row(row, key_prefix):
    cols = st.columns([0.15, 0.45, 0.12, 0.18, 0.1])
    # 1. Costo
    with cols[0]:
    ...
    # 4. Icone + Creste
    with cols[3]:
        ic_html = '<div style="display:flex; gap:1px;">'
        if "Military" in row["icons_list"]: ic_html += SVG_MIL
        if "Intrigue" in row["icons_list"]: ic_html += SVG_INT
        if "Power" in row["icons_list"]: ic_html += SVG_POW
        for cr in row['crest_list']: ic_html += CREST_ICONS.get(cr, "")
        ic_html += '</div>'
...
with c_deck:
    ...
    with col_h: houses = sorted(df[df['card_type'] == 'House']['name'].unique().tolist()); st.session_state.house_choice = st.selectbox("Casata", houses)
    with col_a: agendas = ["Nessuna Agenda"] + sorted(df[df['card_type'] == 'Agenda']['name'].unique().tolist()); st.session_state.agenda_choice = st.selectbox("Agenda", agendas)
    
    m_count, p_count = 0, 0
    with st.container(height=650):
        # Preparazione categorie
        deck_cards = []
        for name, qty in st.session_state.deck.items():
            c_data = df[df['name'] == name].iloc[0].to_dict()
            c_data['qty'] = qty
    ...
    st.download_button("💾 JSON", json.dumps({"House": st.session_state.house_choice, "Agenda": st.session_state.agenda_choice, "Deck": st.session_state.deck}), "mazzo.json", use_container_width=True)
