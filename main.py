...
# CSS personalizzato per rifinire i riquadri e le icone
st.markdown("""
    <style>
    [data-testid="stSidebar"] { min-width: 320px; }
    .stMarkdown, p, label { font-size: 14px !important; }
    
    /* Riduce lo spazio tra i riquadri delle carte */
    [data-testid="stVerticalBlockBorderWrapper"] {
        margin-bottom: -15px;
    }
    
    /* Stile pulsanti: allineamento testo a sinistra per il nome */
    .stButton>button { 
        width: 100%; 
        border-radius: 4px; 
    ...
    .icon-slot { width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; }
    .crest-slot { height: 24px; display: flex; align-items: center; justify-content: start; gap: 4px; }
    </style>
...
# Scudo ingrandito
SVG_SHIELD = """<svg viewBox="0 0 32 32" width="34" height="34"><path d="M16 2 L28 7 V15 C28 22 16 28 16 28 C16 28 4 22 4 15 V7 L16 2 Z" fill="#71797E" stroke="#333" stroke-width="2"/></svg>"""
...
CREST_ICONS = {
    "War": """<svg viewBox="0 0 24 24" width="20" height="20"><path d="M21,16.5L12,22L3,16.5V7.5L12,2L21,7.5V16.5Z" fill="#600" opacity="0.2"/><path d="M18.3,5.7L12,12L13.4,13.4L19.7,7.1L18.3,5.7M5.7,5.7L7.1,4.3L13.4,10.6L12,12L5.7,5.7M12,12L18.3,18.3L16.9,19.7L10.6,13.4L12,12M12,12L5.7,18.3L4.3,16.9L10.6,10.6L12,12Z" fill="#800"/></svg>""",
    "Noble": """<svg viewBox="0 0 24 24" width="20" height="20"><circle cx="12" cy="12" r="8" fill="none" stroke="#D4AF37" stroke-width="2.5"/><circle cx="12" cy="5" r="3" fill="#D4AF37"/></svg>""",
    ...
    "Shadow": """<svg viewBox="0 0 24 24" width="20" height="20"><path d="M12,2A10,10 0 1,0 22,12 10,10 0 0,0 12,2M11,16.5C9.3,16.5 8,15.2 8,13.5H9.5A1.5,1.5 0 0,1 11,15 1.5,1.5 0 0,1 12.5,13.5 1.5,1.5 0 0,1 11,12C9.3,12 8,10.7 8,9A3,3 0 0,1 11,6 3,3 0 0,1 14,9H12.5A1.5,1.5 0 0,1 11,7.5 1.5,1.5 0 0,1 9.5,9 1.5,1.5 0 0,1 11,10.5 3,3 0 0,1 14,13.5 3,3 0 0,1 11,16.5Z" fill="#4A148C"/></svg>"""
}
...
# --- 6. LAYOUT PRINCIPALE ---
c_list, c_view, c_deck = st.columns([2.4, 1.0, 1.6])
...
with c_list:
    ...
    with st.container(height=650):
        for i, row in filtered.head(100).iterrows():
            # Riquadro con bordo per ogni carta
            with st.container(border=True):
                ...
                if row['card_type'] == 'Character':
                    r[2].markdown(f'<div class="svg-container" style="width:34px;height:34px;">{SVG_SHIELD}<span class="svg-text" style="color:white; top:6px; font-size:14px;">{row["strength"]}</span></div>', unsafe_allow_html=True)
