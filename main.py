...
# --- 5. FILTRI SIDEBAR (Invariati) ---
# ... (omessi per brevità ma inclusi nel Main) ...
...
def create_proxy_pdf(deck_dict, house_name, agenda_name, df_all):
    ...
    for i, c_data in enumerate(proxy_list):
        img_path = c_data['preview_image_url']
        full_url = f"https://agot-lcg-search.pages.dev{img_path}"
        
        try:
            # Se la richiesta fallisce, solleva eccezione e ferma il processo
            ...
            # Margine di taglio 0.5mm (Rettangolo grigio chiaro)
            pdf.set_draw_color(200, 200, 200)
            pdf.rect(x - margin_cut, y - margin_cut, c_w + (margin_cut*2), c_h + (margin_cut*2))
            
            pdf.image(img_bytes, x=x, y=y, w=c_w, h=c_h)
            
            col += 1
            if col > 2:
                col = 0
                row += 1
            if row > 2 and i < total - 1:
                pdf.add_page()
                row = 0
                col = 0
        except Exception as e:
            # INTERRUZIONE CRITICA: Se una carta manca, non generiamo il PDF
            raise Exception(f"ERRORE DOWNLOAD: Impossibile scaricare l'immagine per '{c_data['name']}' ({full_url}). Errore: {e}")
    ...
    for label, cond in cat_map:
        subset = [c for c in deck_cards_summary if cond(c)]
        if subset:
            pdf.ln(2)
            ...
            for c in sorted(subset, key=lambda x: x['name']):
                s_name = c['id_str'].split('_')[0].upper()
                line_txt = f" {c['qty']}x {c['name']} ({s_name})"
                # Encode per gestire caratteri speciali senza crash
                pdf.cell(0, 5, line_txt.encode('latin-1', 'replace').decode('latin-1'), ln=True)

    return pdf.output()

# --- 7. RENDERING CARTE (Invariato) ---
...
# ... [Logica Filtri Sidebar e Visualizzazione Libreria qui] ...
# (Mantieni il codice della versione buona per la libreria)

with c_deck:
    # ... [Visualizzazione Deck e Statistiche qui] ...
    # (Mantieni il codice della versione buona per il deck)
